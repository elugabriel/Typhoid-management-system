...
def user_login():
    ...
    if form.validate_on_submit():
        ...
        if user and check_password_hash(user.password_hash, form.password.data):
            login_user(user)
            session['user_type'] = 'user'  # 🔥 mark as user
...
def doctor_login():
    ...
    if form.validate_on_submit():
        ...
        if doctor and doctor.password == form.password.data:
            login_user(doctor)
            session['user_type'] = 'doctor'  # 🔥 mark as doctor
            flash('Login successful!', 'success')
...
def doctor_dashboard():
    # Check if current_user is a Doctor
    if not hasattr(current_user, 'hospital_id'):
        flash("Doctor access only", "danger")
        return redirect(url_for('home'))
    
    # Get consultations with user data
    consultations = db.session.query(Consultation, User)\
